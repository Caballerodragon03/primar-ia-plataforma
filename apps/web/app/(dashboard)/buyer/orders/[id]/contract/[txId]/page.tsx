...
import { ArrowLeft, FileText, CheckCircle2, PenTool, Loader2 } from 'lucide-react';
...
export default function BuyerContractPage() {
  ...
  const handleSign = async () => {
    ...
    try {
      ...
      setContract((prev) => prev ? { ...prev, firmaComprador: signatureData, firmaCompradorFecha: new Date().toISOString() } : prev);
      setShowSignPad(false);
      if (data.data?.bothSigned) {
        alert('Both parties have signed. A QR code has been generated for delivery verification.');
      }
    } catch (err: unknown) {
    ...
    <div className="p-6 max-w-3xl mx-auto space-y-6">
      ...
      {!buyerSigned && (
        <div className="bg-amber-50 border border-amber-200 rounded-card p-5 space-y-4">
          <div>
            ...
            <p className="text-xs text-amber-700 mt-1">Review the contract above, then sign below to proceed. The seller will be notified to countersign.</p>
          </div>
          {!showSignPad ? (
      ...
      {buyerSigned && !sellerSigned && (
        <div className="bg-blue-50 border border-blue-200 rounded-card p-5">
          <p className="text-sm font-semibold text-blue-900">Contract signed</p>
          <p className="text-xs text-blue-700 mt-1">You have signed this contract. Waiting for the seller to countersign.</p>
        </div>
      )}
      ...
      {bothSigned && (
        <div className="bg-green-50 border border-green-200 rounded-card p-5 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-green-700 mt-1">Both parties have signed. The seller will prepare the lot and attach the QR code.</p>
          </div>
